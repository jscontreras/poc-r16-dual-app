...
const InstantSearchApp = ({ searchClient, indexId }: instantSearchParams) => {
  ...
  return (
    <div className="search-is">
      <span className='search-is__app-id'>{`<InstantSearch App> (${indexId})`} </span>
      <InstantSearch searchClient={searchClient} indexName={indexId} routing={false} initialUiState={initialUIState}>
        <Configure hitsPerPage={12} />
        <CustomSearchBox />
        <main>
          <div className="menu">
            <RefinementList attribute="brand" classNames={{ root: 'brand-facets' }} />
